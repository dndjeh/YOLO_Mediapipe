...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        # Process predictions, roi 좌표

        #객체
        #tracked_objects = tracker.update(pred.cpu().numpy())
        
        #객체 추적--------------------------------------------------------------------------
        '''converted_pred=[x_min, y_min, x_max - x_min, y_max - y_min, conf, class]'''
        converted_pred = [[det[0][0].item(), det[0][1].item(), det[0][2].item() - det[0][0].item(), det[0][3].item() - det[0][1].item(), det[0][4].item(), det[0][5].item()] for det in pred]
        print(converted_pred)
        tracked_objects = [tracker.update(obj) for obj in converted_pred]
        ...
        for i, det in enumerate(tracked_objects):  # per image
            ...
            if len(det):
                ...
                # Write results, xyxy = x_min, y_min, x_max, y_max
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:
                        ...
                        xy = (int(xyxy[0]), int(xyxy[1])) #ROI영역의 x_min, y_min
                        p2 = (int(xyxy[2]), int(xyxy[3])) #ROI영역의 x_max, y_max
                        #annotator.text(xy, f"ROI: {xy[0]}, {xy[1]}, {p2[0]}, {p2[1]}", txt_color=(0,0,0), anchor='bottom', box_style=True)
                        center_x = (xy[0] + p2[0]) // 2
                        center_y = (xy[1] + p2[1]) // 2
                
                        # ROI 영역을 text로 나타내기
                        cv2.putText(im0, f"ROI: {xy[0]}, {xy[1]}, {p2[0]}, {p2[1]}", (center_x, center_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
